...
holds scan-target repos, producer threat-model exports, and external oracles.
It is **not** an application — there is nothing to build or deploy here. Read
`README.md` first.

The fixture repos under `repos/` are **plain directories in this repo**, not
submodules. Add or edit fixture files directly and commit them here.
...
- **Add a producer service:** create `repos/cross-repo-threat-fixture/<svc>/`
  with the source files and a `docs/security/threat-model.yaml` export, update
  the consumer's `related-repos.yaml`, and add the name + interface to
